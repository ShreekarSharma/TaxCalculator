function calculateFinalAmount() {
  ...
  if (totalAmount <= 800000) {
    ...
    document.getElementById("result").textContent = finalAmount;
  } else if (ageGroup === '<40') {
...
grossAnnualIncomeInput.addEventListener("input", () => {
  ...
  if (!numberPattern.test(inputValue)) {
    document.querySelector('.gross-annual-income-icon').classList.remove('hide');
...
const totalApplicableDeductionsInput = document.getElementById("total-applicable-deductions");
totalApplicableDeductionsInput.addEventListener("input", () => {
  const inputValue = totalApplicableDeductionsInput.value;
  const numberPattern = /^\d*$/;
  if (!numberPattern.test(inputValue)) {
    document.querySelector('.total-applicable-deductions-icon').classList.remove('hide');
    totalApplicableDeductionsInput.classList.remove('green-outline');
    totalApplicableDeductionsInput.classList.add('red-outline');
    submitBtn.setAttribute('disabled','disabled');
  } else{
    document.querySelector('.total-applicable-deductions-icon').classList.add('hide');
